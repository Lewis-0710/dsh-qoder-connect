	...
	factory: (require) => {
		...
		*/
		...
		function stableProject(scope) {
			...
			if (cachedProjection === void 0 || cachedScope !== scope || cachedProjection.status !== next.status || cachedProjection.writable !== next.writable || cachedProjection.values.sidebarQuotaCN !== next.values.sidebarQuotaCN || cachedProjection.values.sidebarQuotaGlobal !== next.values.sidebarQuotaGlobal || cachedProjection.values.quotaPollMs !== next.values.quotaPollMs) {
				cachedScope = scope;
		...
		*/
		...
		function QoderPluginCard(props) {
			...
			const cnDotStatus = isUnified && activeVariantId === "qoder" ? status === void 0 ? "loading" : status.status : reported.cn ? "signed-in" : "signed-out";
			const globalDotStatus = isUnified && activeVariantId === "qoder-global" ? status === void 0 ? "loading" : status.status : reported.global ? "signed-in" : "signed-out";
