...
function qoderEncodeBody(plaintext) {
	...
	for (let i = 0; i < n; i++) {
		...
		if (c === "=") out += "$";
			...
			if (idx >= 0) out += qoderCustomAlphabet[idx];
			else out += c;
		}
	}
	return out;
...
function parseEnvelope(rawData) {
	...
	if (envelope.statusCodeValue !== void 0 && envelope.statusCodeValue !== 200) throw qoderHttpError(`Qoder service returned upstream error status ${envelope.statusCodeValue}.`, { status: envelope.statusCodeValue });
...
var DefaultQoderTransport = class {
	...
	async *generate(options, model) {
		...
		yield* streamQoderChat(options, model, await this.auth.getCredentials(pat, options.signal), messages, {
			fetch: this.fetchImpl,
			logger: this.logger,
			region: this.region,
...
const QODER_CONNECT_VERSION = "0.1.0";
//#endregion
