...
const DISABLED_MODELS_FIELD = z.array(z.string()).default([]).description("Models hidden from the provider catalog and picker");
...
const AUTO_CHECK_IN_FIELD = z.boolean().default(false).description("每天自动签到领取算力额度（默认关闭）");
/**
...
*/
const CHECK_IN_MINUTE_FIELD = z.number().default(600).min(0).max(1439).description("每日自动签到的时刻（自 UTC+8 午夜起的分钟数，600 = 10:00）");
/**
...
function pickFields(source, keys) {
	const value = source();
	const picked = {};
	for (const key of keys) if (value[key] !== void 0) picked[key] = value[key];
	return picked;
}
...
function createVariantRuntime(ctx, config, variant, current, identityOf) {
	...
	if (variant.id !== CHINA_VARIANT.id) {
		...
		if (config.disabledModels !== void 0) catalog.setDisabledModels(config.disabledModels);
	} else {
		...
		if (config.disabledModelsCN !== void 0) catalog.setDisabledModels(config.disabledModelsCN);
	}
	...
	*/
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		settingsCtx.settings.installSection(ctx, QODER_QUOTA_SETTINGS_NS, QUOTA_SECTION, config, {
			...
			onChange: () => {
				checkInScheduler.rearm();
				checkInScheduler.sweepAll(true);
			}
		});
		setMaximumContextWindow = async (enabled) => {
