	...
	*/
	...
	async sweepAll(isCatchUp, only) {
		...
		for (const target of this.targets) {
			...
			if (record?.lastDate === today && (record.status === "claimed" || record.status === "already-claimed")) continue;
...
const GLOBAL_SECTION_KEYS = ["useMaximumContextWindow", "modelContextWindows"];
