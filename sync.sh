...
#   - 文件：sync.patch（定制修改的干净快照，已排除 sync.patch/sync.sh/MODIFICATIONS.md 自身）
#
# 同步策略：
#   ── 策略 1：Patch Apply（优先）──────────────────────────────
...
#     3. 对 MODIFICATIONS.md：保留本地维护文档
#     4. 对其它冲突文件：列出差异并以本地版本（--ours）为准
#     5. 提交合并结果
#
#   ── 收尾工作：
#     1. 重新生成干净的 sync.patch（自动排除 sync.patch、sync.sh 与 MODIFICATIONS.md）
#     2. 运行自动化构建与客户端测试验证
...
# Step 0: 创建安全临时备份目录
TMP_DIR="$(mktemp -d /tmp/dsh_qoder_sync_XXXXXX)"
cleanup() {
    rm -rf "$TMP_DIR"
}
...
if [ -f "$SCRIPT_DIR/MODIFICATIONS.md" ]; then
    cp "$SCRIPT_DIR/MODIFICATIONS.md" "$TMP_DIR/MODIFICATIONS.md"
fi
...
if [ -s "$TMP_DIR/sync.patch" ]; then
    ...
    if git apply --check "$TMP_DIR/sync.patch" 2>/dev/null; then
        ...
        if [ -f "$TMP_DIR/MODIFICATIONS.md" ]; then
            cp "$TMP_DIR/MODIFICATIONS.md" "$SCRIPT_DIR/MODIFICATIONS.md"
        fi
        git add -A
        ...
        for f in $CONFLICTS; do
            case "$f" in
                README*|readme*)
                    ...
                    git add "$f"
                    ;;
                sync.sh|sync.patch|MODIFICATIONS.md)
...
if [ "$SKIP_TEST" = false ]; then
    if command -v pnpm >/dev/null 2>&1; then
        ...
        echo "🔍 正在运行项目构建与测试验证..."
        pnpm run build
        pnpm vitest run tests/client-fallback.spec.ts tests/version.spec.ts || true
    fi
fi

# Step 4: 重新生成干净的 sync.patch（排除 sync.patch、sync.sh 和 MODIFICATIONS.md）
echo "[4/4] 重新生成干净的 sync.patch..."
git diff upstream/main...custom ':!sync.patch' ':!sync.sh' ':!MODIFICATIONS.md' > "$PATCH_FILE"
PATCH_LINES=$(wc -l < "$PATCH_FILE" | tr -d ' ')
echo "✅ sync.patch 生成完毕 (共 $PATCH_LINES 行，已排除同步脚本、快照与修改文档自身)"

# 确保 sync.sh 和 sync.patch 都被暂存并提交
git add sync.patch sync.sh MODIFICATIONS.md
