...
import { fingerprintModel, type QoderProbeRecord, type QoderProbeStore } from './probe-store.ts'
import type { QoderUpstreamClient } from './upstream.ts'

/** What the caller learns about a completed probe. */
export type QoderProbeStatus =
  | { state: 'ok'; validation: QoderProbeRecord['validation']; efforts: readonly string[]; requests: number }
  | { state: 'unavailable'; reason: string }

/** Options for {@link QoderProbeService}. */
export interface QoderProbeServiceOptions {
 ...
 */
export class QoderProbeService {
  private readonly options: QoderProbeServiceOptions
  private queue: Promise<unknown> = Promise.resolve()
  private readonly pending = new Map<string, Promise<QoderProbeStatus>>()
   ...
   */
  async probe(modelId: string, manualConsent = false): Promise<QoderProbeStatus> {
    ...
    const account = this.options.account()
    if (account === undefined) return { state: 'unavailable', reason: 'no Qoder credential' }
    const pendingKey = JSON.stringify([account, modelId])
    const pending = this.pending.get(pendingKey)
    if (pending !== undefined) return pending

    const run = this.queue.then(async (): Promise<QoderProbeStatus> => {
      ...
      if (credential === undefined) return { state: 'unavailable', reason: 'no Qoder credential' }

      const send = this.options.send === undefined
        ? (effort: string | undefined, signal: AbortSignal) =>
            this.options.client.probeEffort(modelId, effort, signal)
