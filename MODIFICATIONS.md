...
- **涉及文件**：
  ...
  - `lib/client.js` 等编译产物
- **改动详情**：
  1. **移除独立一级菜单**：删除在 `settings.section` 槽位上注册的 `plugin-settings` 分区及其容器组件 `PluginSettingsSection`，删除 `joinSharedSettingsBlock` 逻辑。
  2. **恢复原生插件配置插槽**：使用 `ctx.slots.inject('settings.plugin.item', ...)` 将统一配置卡片 `QoderPluginCard` 注册到原生插件配置区（key 为 `'qoder'`，priority 为 50）。
  3. **单元测试与镜像同步**：更新 `tests/client-fallback.spec.ts` 镜像测试，确保捕获的 console 错误数、注册 seats 与新架构完全吻合，清除遗留的历史敏感标识，确保所有测试 100% 通过。
  4. **重新编译打包**：执行 `pnpm run build`，更新 `lib/` 目录下最新的客户端打包脚本。

---

### 修改记录 2：插件自有配置本地原子读写（OwnQuotaSettingsScope）

- **修改目的**：
  避免 DSH 0.1.7 中宿主 `configForms` 写配置时触发整树 reconcile 和长时间热重载（约 1~1.5 秒且刷新全部客户端镜像），提升配置持久化速度与稳定性。
- **涉及文件**：
...
- **修改日期**：2026-09-26
- **修改目的**：
  在将客户端入口切回「设置 - 插件 - 插件配置」（`settings.plugin.item`）后，DSH 原生配置页组件（`ConfigurablePluginsTab`）基于“两账本交集（intersection）”原理运行：它仅渲染其 `key` 存在于宿主 `served namespaces` 列表中的卡片。此前重构由于移除了 `installSection` 导致宿主未宣告 `qoder` 命名空间，前端将其误判为未接入而过滤。
- **涉及文件**：
  - `src/index.ts`
  - `lib/index.js`
- **改动详情**：
  - 在 `src/index.ts` 的 `ctx.inject(['settings'], ...)` 中恢复通过 `settingsService.installSection` 注册 `QODER_SETTINGS_NS`（`qoder`）、`QODER_GLOBAL_SETTINGS_NS` 与 `QODER_QUOTA_SETTINGS_NS`。
...
2. **手动合并步骤（备用）**：
   ...
   # 5. 重新编译与测试
   pnpm run build
   pnpm vitest run tests/client-fallback.spec.ts tests/version.spec.ts
